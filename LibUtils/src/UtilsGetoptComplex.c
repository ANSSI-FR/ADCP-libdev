...
#define EXPORTS_GETOPT // MODIFIED
#include "UtilsGetoptComplex.h" // MODIFIED
	...
	if (longopts != NULL && (argv[d->optind][1] == '-' || (long_only && (argv[d->optind][2] || !strchr(optstring, argv[d->optind][1])))))
	{
		...
		for (p = longopts, option_index = 0; p->name; p++, option_index++)
			if (!strncmp(p->name, d->__nextchar, namelen))
			{
				...
				{
// C6255	Unprotected use of alloca	_alloca indicates failure by raising a stack overflow exception.Consider using _malloca instead.AceFilter	getopt.c	223
// C6263	Using alloca in loop	Using _alloca in a loop:  this can quickly overflow stack.	AceFilter	getopt.c	226
// not so important
	...
	if (longopts != NULL && (argv[d->optind][1] == L'-' || (long_only && (argv[d->optind][2] || !wcschr(optstring, argv[d->optind][1])))))
	{
		...
		for (p = longopts, option_index = 0; p->name; p++, option_index++)
			if (!wcsncmp(p->name, d->__nextchar, namelen))
			{
				...
				{
// C6255	Unprotected use of alloca	_alloca indicates failure by raising a stack overflow exception.Consider using _malloca instead.AceFilter	getopt.c	683
// C6263	Using alloca in loop	Using _alloca in a loop: this can quickly overflow stack.AceFilter	getopt.c	686
// not so important
#pragma warning(suppress: 6255)
#pragma warning(suppress: 6263)
					struct option_list *newp = (struct option_list*)alloca(sizeof(*newp));
