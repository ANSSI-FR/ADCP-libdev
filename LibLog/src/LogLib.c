...
BOOL LogiFormatBuffer(
    ...
    do {
        dwCharLen += LOG_DEFAULT_BUFFER_LEN;
        ptTmpBuff = UtilsHeapAllocOrReallocHelper(gs_pLogHeap, ptTmpBuff, SIZEOF_TSTR(dwCharLen));
        iSize = _vstprintf_s(ptTmpBuff, dwCharLen, ptFormat, vaArgs);
    } while (iSize == -1);

    if (iSize + LOG_TIME_LEN >= (int)dwCharLen) {
        dwCharLen += LOG_TIME_LEN;
    }
    ptTmpBuffWithTime = UtilsHeapAllocStrHelper(gs_pLogHeap, dwCharLen);
    iSize = _stprintf_s(ptTmpBuffWithTime, dwCharLen, LOG_TIME_FRMT, sTime.wHour, sTime.wMinute, sTime.wSecond, ptTmpBuff);
